<div class="modal-content">
    <div class="modal-header">
        <h3 class="modal-title">Edit Level Two Category</h3>
    ...
    <form role="form" method="POST" action="{{ route('level-two-categories.update', $level_two_category->id) }}">
        @csrf
        @method('Patch')
        <div class="modal-body">
            ...
            <div class="row mt-4">
                {{--id--}}
                <input type="hidden" name="id" value="{{ $level_two_category->id }}">

                {{--Name--}}
                <div class="col-12 mb-6">
                    <label for="name" class="required form-label">Name</label>
                    <input type="text" id="name" name="name" class="form-control" placeholder="Name" required
                           value="{{ $level_two_category->name }}"/>
                    @error('name')
                ...
                <div class="col-sm-12 mb-6">
                    ...
                    <select id="status" name="status" class="form-select" aria-label="Select status" required>
                        ...
                        <option value="1" @if($level_two_category->status == 1) selected @endif>Active</option>
                        <option value="0" @if($level_two_category->status == 0) selected @endif>Inactive</option>
                    </select>
                    @error('status')
